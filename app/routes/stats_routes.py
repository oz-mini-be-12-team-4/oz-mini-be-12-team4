...
stats_routes_blp = Blueprint("stats_routes", __name__, url_prefix='/stats', description='Operations on stats')
    ...
    try:
        result = (
            db.session.query(
                ...
                (
                    func.count(Answer.id) * 100 / func.sum(func.count(Answer.id)).over()
                ).label("percentage"),
            )
        ...
        result = (
            db.session.query(
                ...
                (
                    ...
                    * 100
                    / func.sum(func.count(Answer.id)).over(partition_by=Question.id)
                ).label("percentage"),
            )
            .join(Choices, Choices.id == Answer.choice_id)
            .join(Question, Question.id == Choices.question_id)
